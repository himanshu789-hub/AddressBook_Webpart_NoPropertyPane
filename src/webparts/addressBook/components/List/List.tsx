...
class Item extends React.Component<ItemProps, {}> {
	...
	render() {
		return (
			<NavLink to={'/' + this.props.item.Id} isActive={(match, location) => {
				console.log('M : ' ,match);
...
export class List extends React.Component<IListProps & IListDependenciesProps, IListState> {
	constructor(props) {
		super(props);
	...
	componentDidMount() {
		const { AddressBookService, IsStateSetFromGetByList,setStatus,spHttpClient, siteUrl } = this.props;
	...
	public render() {
		...
		for (var i:number = 0; i < contacts.length; i++) {
			items.push(<li key={contacts[i].Id}><Item item={contacts[i]}/></li>);
		}
